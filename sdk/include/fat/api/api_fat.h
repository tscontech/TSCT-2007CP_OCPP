/***************************************************************************
 *
 *            Copyright (c) 2003-2019 by HCC Embedded
 *
...
#if ( VER_MDRIVER_MAJOR != 1 ) || ( VER_MDRIVER_MINOR < 3 )
 #error "VER_MDRIVER invalid"
#endif
...
#define F_MAXLNAME FN_MAXLNAME
 ...
 #if ( F_FILE_CHANGED_EVENT != 0 )

/* If file changed event feature is enabled, path shall be able to store the */
/* drive letter and a colon (e.g. "A:") and the string terminator */
/* character as well. */
  #define F_MAXDRIVENAME      2
  #define F_STRINGEND         1
  #define F_MAXPATHNAME       ( FN_MAXPATHNAME + F_MAXDRIVENAME + F_STRINGEND )
 #else
...
#if ( F_DIR_WALK_SUPPORT == 0 )
 #define F_DISK_USAGE_SUPPORT 0
#endif
...
typedef struct
{
  ...
  uint32_t  sectorend;          /* last sector position of the cluster */
...
/****************************************************************************
 *
 * Callback function used by f_dir_walk()
 *
 * INPUTS
 *
 * p_find - pointer to structure used by findfirst()/findnext()
 * depth - Directory depth counted from base directory
 * p_user_data - pointer to user's data
 *
 * RETURNS
 *
 ...
 *
 ***************************************************************************/
typedef int  t_dir_walk_cb_fn ( FN_FIND * p_find, uint16_t depth, void * p_user_data );
...
/****************************************************************************
 *
 * Callback function used by f_wdir_walk()
 *
 * INPUTS
 *
 * p_find - pointer to structure used by wfindfirst()/wfindnext()
 * depth - Directory depth counted from base directory
 * p_user_data - pointer to user's data
 *
 * RETURNS
 ...
 *
 ***************************************************************************/
typedef int  t_wdir_walk_cb_fn ( FN_WFIND * p_find, uint16_t depth, void * p_user_data );
#endif
 ...
 ***************************************************************************/
 ...
 #define f_get_cluster_size( drivenum, p_cluster_size_byte )            fm_get_cluster_size( drivenum, p_cluster_size_byte )
 #if ( F_DISK_USAGE_SUPPORT != 0 )
  #define f_disk_usage( filename, p_find, disk_usage_byte )             fm_disk_usage( filename, p_find, disk_usage_byte )
 #endif
 #if ( F_DIR_WALK_SUPPORT != 0 )
  #define f_dir_walk( filename, p_find, p_callback, p_user_data )       fm_dir_walk( filename, p_find, p_callback, p_user_data )
 #endif
#endif /* if ( !FN_CAPI_USED ) */
  ...
  #if ( F_DISK_USAGE_SUPPORT != 0 )
   #define f_wdisk_usage( filename, p_find, disk_usage_byte )       fm_wdisk_usage( filename, p_find, disk_usage_byte )
  #endif
  #if ( F_DIR_WALK_SUPPORT != 0 )
   #define f_wdir_walk( filename, p_find, p_callback, p_user_data ) fm_wdir_walk( filename, p_find, p_callback, p_user_data )
  #endif
 #endif /* if ( !FN_CAPI_USED ) */
...
int fm_get_cluster_size ( int drivenum, uint32_t * p_cluster_size_byte );

#if ( F_DISK_USAGE_SUPPORT != 0 )
int fm_disk_usage ( const char * filename, FN_FIND * p_find, uint64_t * p_disk_usage_bytes );
#endif
#if ( F_DIR_WALK_SUPPORT != 0 )
int fm_dir_walk ( const char * filename, FN_FIND * p_find, t_dir_walk_cb_fn * p_callback, void * p_user_data );
#endif
...
int fm_wsetattr ( const wchar * filename, unsigned char attr );
 #if ( F_DISK_USAGE_SUPPORT != 0 )
int fm_wdisk_usage ( const wchar * filename, FN_WFIND * p_find, uint64_t * p_disk_usage_bytes );
 #endif
 #if ( F_DIR_WALK_SUPPORT != 0 )
int fm_wdir_walk ( const wchar * filename, FN_WFIND * p_find, t_wdir_walk_cb_fn * p_callback, void * p_user_data );
 #endif
#endif /* if HCC_UNICODE */
