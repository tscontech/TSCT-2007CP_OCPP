        ...
        if( ret == MBEDTLS_ERR_SSL_TIMEOUT )
        {
            ...
            if( ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER )
            {
                ...
                if( ( ret = mbedtls_ssl_resend( ssl ) ) != 0 )
                {
                    ...
                    printf("[mbedtls_ssl_fetch_input] mbedtls_ssl_resend() : %04x\r\n", -ret);
                    return( ret );
                }
                     ...
                     ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_PENDING )
            {
                if( ( ret = ssl_resend_hello_request( ssl ) ) != 0 )
                {
                    ...
                    printf("[mbedtls_ssl_fetch_input] ssl_resend_hello_request() : %04x\r\n", -ret);
                    return( ret );
                }
                printf("[mbedtls_ssl_fetch_input] ssl_resend_hello_request()2 : MBEDTLS_ERR_SSL_WANT_READ\r\n");
                return( MBEDTLS_ERR_SSL_WANT_READ );
            }
#endif /* MBEDTLS_SSL_SRV_C && MBEDTLS_SSL_RENEGOTIATION */
        }
        ...
        while( ssl->in_left < nb_want )
        {
            ...
            else
            {
                if( ssl->f_recv_timeout != NULL )
                {
                    ...
                    printf("[mbedtls_ssl_fetch_input] f_recv_timeout() : %04x\r\n", -ret);                                               
                }
                else
                {
                    ret = ssl->f_recv( ssl->p_bio,
                                       ssl->in_hdr + ssl->in_left, len );
                    printf("[mbedtls_ssl_fetch_input] f_recv() : %04x\r\n", -ret);                   
                }
            }
            ...
            if( ret == 0 ) {
                printf("[mbedtls_ssl_fetch_input] ssl->f_recv or timeout() : MBEDTLS_ERR_SSL_CONN_EOF\r\n");
                return( MBEDTLS_ERR_SSL_CONN_EOF );
            }

            if( ret < 0 ){
                printf("[mbedtls_ssl_fetch_input] ssl->f_recv or timeout() : %04x\r\n", -ret);
                return( ret );
            }
    ...
    if( ssl->keep_current_message == 0 )
    {
        do {
            ...
            if( ret != 0 )
            {
                ...
                printf("[mbedtls_ssl_read_record] mbedtls_ssl_read_record_layer() : %04x\r\n", -ret);
                return( ret );
            }
        ...
        if( 0 != ret )
        {
            ...
            printf("[mbedtls_ssl_read_record] mbedtls_ssl_handle_message_type() : %04x\r\n", -ret);
            return( ret );
        }
...
int mbedtls_ssl_read_record_layer( mbedtls_ssl_context *ssl )
{
    ...
    if( ( ret = mbedtls_ssl_fetch_input( ssl, mbedtls_ssl_hdr_len( ssl ) ) ) != 0 )
    {
        ...
        printf("[mbedtls_ssl_read_record_layer] mbedtls_ssl_fetch_input() : %04x\r\n", -ret);
        return( ret );
    }
...
#endif
        printf("[mbedtls_ssl_read_record_layer] ssl_parse_record_header() : %04x\r\n", -ret);
    ...
    if( ( ret = ssl_prepare_record_content( ssl ) ) != 0 )
    {
        ...
        if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
        {
            ...
            printf("[mbedtls_ssl_read_record_layer] ssl_prepare_record_content() : %04x\r\n", -ret);
            return( ret );
        }
        else
#endif
    ...
    if( ssl->in_msgtype == MBEDTLS_SSL_MSG_ALERT )
    {
        ...
        if( ssl->in_msg[0] == MBEDTLS_SSL_ALERT_LEVEL_FATAL )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "is a fatal alert message (msg %d)",
                           ssl->in_msg[1] ) );
            printf("[mbedtls_ssl_handle_message_type] fatal alert message (msg %d)\r\n", ssl->in_msg[1]);
