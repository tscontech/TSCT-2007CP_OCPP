...
/* Define a Macro that returns TRUE if the chipset has a 2045 core (includes 2048) */
#define BTM_IS_BRCM_2045_CORE() (((btm_cb.devcb.local_version.manufacturer) == LMP_COMPID_BROADCOM) &&   \
                              ((btm_cb.devcb.local_version.hci_version) == HCI_PROTO_VERSION_2_0) ? TRUE : FALSE)

/* Define a Macro that returns TRUE if the chipset has a 2046 core */
#define BTM_IS_BRCM_2046_CORE() (((btm_cb.devcb.local_version.manufacturer) == LMP_COMPID_BROADCOM) &&   \
                              ((btm_cb.devcb.local_version.hci_version) == HCI_PROTO_VERSION_2_1) ? TRUE : FALSE)


/* Needed to check for some BMW models which use an old Broadcom chip */
#define BTM_IS_REMOTE_BRCM_203X(rm, rlmp, rlmpsub) ( ( ((rm) == LMP_COMPID_BROADCOM) &&   \
                            ((rlmp) == 1) ) ? TRUE : FALSE)
...
typedef struct
{
    ...
    tBTM_BLE_PHY_INFO_CBACK        *p_le_read_phy_cmpl_cb;
    tBTM_BLE_PHY_INFO_CBACK        *p_le_phy_update_cmpl_cb;
...
#define BTM_BLE_ADDR_STATIC         2   /* static_addr  */
    ...
    BOOLEAN             current_addr_valid; /* current addr info is valid or not*/
#endif
} tBTM_SEC_BLE;
...
typedef struct
{
    ...
    tBTM_LOC_BD_NAME bd_name_le;                 /* local Bluetooth LE device name */
#endif
...
#if (BRCM_LPST_INCLUDED == TRUE)
#define BTM_LPST_CB_ACL_CONN        0x01    // for tACL_CONN
#define BTM_LPST_CB_SEC_DEV         0x02    // for tBTM_SEC_DEV_REC
#define BTM_LPST_CB_LINK_KEY        0x04    // link key has changed
#define BTM_LPST_CB_BD_ADDR         0x08    // for BD_ADDR (link down)
// BTM_LPST_CB_REQUIRED is the CBs sent in btm_lpst_send_control_blocks()
#define BTM_LPST_CB_REQUIRED        0x0F    // bitmask of all of the above
    /* leave 0x10 open for L2C_LPST_CB_CACHE_OK */
#define BTM_LPST_CB_BTA             0x20    // for BTA
#define BTM_LPST_CB_BR_CONN         0x40    // set to indicate BR is connected
    UINT16                  lpst_cb_required;
    UINT16                  lpst_cb_sent;
    BD_ADDR                 lpst_bd_addr;   /* The BD address of the remote (not bridge) */
#endif
#endif
...
/* Internal functions provided by btm_lpst.c (for LPST support)
**********************************************
*/
#if (BRCM_LPST_INCLUDED == TRUE)
typedef struct
{
    BD_ADDR lpst_bda;             /* LPST bridge peer device */
    UINT16  lpst_lcid[BTM_LPST_MAX_LCID_NUM];  /* traffic CID to be forwarded in bridge */
    BOOLEAN sync_latecomer;       /* TRUE, if syncing control block for late comer */
    UINT8   lpst_role;            /* LPST bridge role: BTM_LPST_ROLE_PE/BTM_LPST_ROLE_SE/NONE */
    UINT16  lpst_media_cid;       /* AVK media channel CID */
    UINT16  lpst_bridge_handle;   /* LPST bridge LE link handle */
    UINT8   le_link_role;         /* local devcice LE link role */
    UINT8   le_events;            /* The LE event masks LPST is interested */
    BOOLEAN le_role_switch_pending;
    BD_ADDR old_br_bda;           /* addr for old BR connection */
    UINT16  old_br_handle;        /* old BR connection - disconnecting */
    UINT16  lpst_shadow_handle;   /* LPST shadow connection HCI handle */
    UINT16  lpst_event_count;     /* LPST audio sync event counter */

#define BTM_LPST_SHADOW_WAIT_NONE       0    /* no shadow request pending*/
#define BTM_LPST_SHADOW_WAIT_CMPL       0x01 /* shadow link request start, pending completion */
#define BTM_LPST_SHADOW_WAIT_START      0x02 /* shadow link request waiting to be started */
    UINT8           lpst_shadow_wait;        /* Waiting for shadow connection */
    TIMER_LIST_ENT  lpst_shadow_tle;

    UINT8           lpst_single_mode;     /* LPST single mode */
    UINT8           lpst_sync_on;         /* LPST audio sync on or not */
    UINT8           lpst_vsc_retry;       /* LPST audio sync or shadow retry number */
    BOOLEAN         lpst_disabled;        /* if TRUE, can not use LPST. Report LPST Disabled*/
    UINT8           need_role_cfg;        /* BTM_LPST_SEND_ROLE_CFG | BTM_LPST_WAIT_ROLE_CFG */
    UINT16          cblk_required;

    tBTM_LPST_DATA_HANDLER *lpst_data_handler;  /* data handler for app */
    tBTM_LPST_SYNC_HANDLER *p_cblk_cback;       /* control block handler for bta */
    tBTM_LPST_SYNC_CMPL_CBACK   *p_sync_cback;  /* for AVDT sync process */
    void                        *p_sync_ref;    /* AVDT sync reference */

    UINT8                   role_switch_state;
    TIMER_LIST_ENT          lpst_switch_tle;
    tBTM_LPST_INT_EVT_CBACK *lpst_int_evt_cback;/* event handle for bta */
    tBTM_LPST_EVENT_CBACK   *lpst_event_handler;/* event handle for app */

    BOOLEAN                 lpst_fwd_rfcomm; /* current flag for rfcomm data forwarding */
    UINT8                   lpst_fwd_scn;   /* forward SCN number */
    UINT8                   pm_id; /* for power manager */
    BOOLEAN                 is_disabled;    /* TRUE, if LPST bridge is disabled */

} tBTM_LPST_BRIDGE_CB;
extern tBTM_LPST_BRIDGE_CB  btm_lpst_bridge_cb;

enum
{
    BTM_LPST_ACL_DOWN_EVENT,
    BTM_LPST_LINK_KEY_EVENT,
    BTM_LPST_SEC_SERVICE_EVENT
};

extern void btm_lpst_sync_control_block (BD_ADDR bd_addr, UINT8 btm_lpst_event);
extern void btm_lpst_clear_cb (void);
#endif
