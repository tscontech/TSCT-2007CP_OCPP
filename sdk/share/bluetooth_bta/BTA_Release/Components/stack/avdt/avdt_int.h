...
#define AVDT_TCID_SIG       0 /* the TCID for signal channel is 0 */
...
#if (BTU_BTC_SNK_INCLUDED == TRUE)
    AVDT_CCB_API_BTC_START_RSP_EVT,
#endif
...
typedef struct {
    ...
    tAVDT_MSG           msg;            /* start response message */
} tAVDT_CCB;
...
typedef struct {
    ...
    UINT8           lcid_tbl[L2C_MAX_CHANNEL_MAP];   /* map LCID to tc_tbl index */
} tAVDT_AD;
...
#define AVDT_LPST_CCB_DONE     0x01
    UINT8               lpst_cb_flags;
    UINT8               lpst_ccb_idx;                   /* the index for ccb on LPST */
#endif
#if ((BTU_DUAL_STACK_MM_INCLUDED == TRUE) || (BTU_DUAL_STACK_BTC_INCLUDED == TRUE)) && (BTU_STACK_LITE_ENABLED == FALSE)
    tBTM_SYNC_CPLT_CBACK p_sync_cplt_cback;
    UINT8               avdt_handle[BTM_SYNC_INFO_NUM_STR];
#endif
#if (BTU_MULTI_AV_INCLUDED == TRUE)
    BOOLEAN             ma_cong;        /* Whether multi-av is congested */
    BT_HDR              *p_ma_pkt;      /* packet waiting to be sent */
    UINT16              ma_seq;         /* media packet sequence number */
#endif
} tAVDT_CB;
...
#if (BTU_BTC_SNK_INCLUDED == TRUE)
extern void avdt_ccb_snd_btc_start_rsp(tAVDT_CCB *p_ccb, tAVDT_CCB_EVT *p_data);
#endif
...
/* Internal functions provided by avdt_lpst.c (for LPST support)
**********************************************
*/

#if (BRCM_LPST_INCLUDED == TRUE)
#define AVDT_LPST_CB_CCB            0x01    // for tAVDT_CCB
#define AVDT_LPST_CB_CCB_ADDR       0x02    // to identify tAVDT_CCB
#define AVDT_LPST_CB_AD             0x03    // for tAVDT_AD
#define AVDT_LPST_CB_SCB            0x04    // for scb handle + tAVDT_SCB
#define AVDT_LPST_CB_CLOSE          0x05    // for lcid
#define AVDT_LPST_CB_COMMAND        0x06    // for p_ccb->p_curr_cmd


extern void avdt_lpst_sync_abort(tAVDT_CCB *p_ccb);
extern void avdt_lpst_forward_cmd (tAVDT_CCB *p_ccb, BT_HDR *p_msg);
extern void avdt_lpst_send_control_blocks (tAVDT_SCB*p_scb);
extern void avdt_lpst_send_close (UINT16 lcid, UINT8 tcid);
extern void avdt_lpst_sync_complete (UINT8 status, void *p_ref);
#endif
