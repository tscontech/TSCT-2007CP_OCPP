...
#define LPST_HEADER_LEN ((UINT16)(L2CAP_MIN_OFFSET + 1))
...
typedef struct
{
    ...
    UINT8           lpst_lcb_idx;                   /* non-0, if the index for lcb on LPST */
    UINT32          lpst_sent_mask;                 /* the tL2C_CCB already sent to SE  */
    UINT32          connected_mask;                 /* all connected L2CAP channels */
#endif
...
#if (BTU_STACK_LITE_ENABLED == TRUE)
    BOOLEAN             is_lite_segment;
#endif

#if (BTU_DUAL_STACK_MM_INCLUDED == TRUE) || (BTU_DUAL_STACK_BTC_INCLUDED == TRUE) || (BTU_STACK_LITE_ENABLED == TRUE)
    UINT16          controller_xmit_quota;          /* total ACL quota of full stack or light stack  */
    UINT16          controller_xmit_window_adjust;  /* unacked packets when quota is reduced  */
#endif

#if (BTU_DUAL_STACK_BTC_INCLUDED == TRUE)
    /* for burst mode playback */
    tL2CA_RESERVE_CMPL_CBACK    *p_controller_xmit_window_reserve_cback;
    UINT8                       controller_xmit_window_reserve_adjust;
    UINT8                       controller_xmit_window_reserve_num;
#endif

#if (BTU_DUAL_STACK_MM_INCLUDED == TRUE) || (BTU_DUAL_STACK_BTC_INCLUDED == TRUE)
    UINT16          light_xmit_quota;               /* Total ACL window for all links in light stack   */
    tBTM_SYNC_CPLT_CBACK p_sync_cplt_cback;
    tL2CA_SYNC_STATE     sync_state;
    UINT8           auto_sync_unacked;              /* total outstanding auto sync requests */
#endif

#if (BTU_MULTI_AV_INCLUDED == TRUE)
    BUFFER_Q        multi_av_data_q;                /* Multi-AV payloads */
    UINT16          multi_av_cid_list[L2CAP_MAX_MULTI_AV_CID];  /* list of media channels */
    UINT8           multi_av_cid_bitmap;            /* each bit on/off for CID in list */
    UINT8           multi_av_num_cid;
    ...
    BOOLEAN         multi_av_data_cong_sent;        /* TRUE if Multi-AV congested */
#endif
    ...
    UINT16          dyn_psm;
    UINT16          first_cid;      /* first available CID on local device */
} tL2C_CB;
...
extern void     l2c_link_sec_comp (BD_ADDR p_bda, BOOLEAN trasnport, void *p_ref_data, UINT8 status);
extern void     l2c_link_segments_xmitted (BT_HDR *p_msg);
extern void     l2c_pin_code_request (BD_ADDR bd_addr);
extern UINT8    l2c_pin_get_link_role (BD_ADDR bd_addr);
...
/* Internal functions provided by l2c_lpst.c (for LPST support)
**********************************************
*/

#if (BRCM_LPST_INCLUDED == TRUE)
#define L2C_LPST_CB_LCB_BR          0x01    // for tL2C_LCB/BR
#define L2C_LPST_CB_INFO            0x02    // tL2C_LPST_INFO_CB (psm, first_cid)
#define L2C_LPST_CB_CCB             0x04    // for tL2C_CCB
#define L2C_LPST_CB_ADDR_BR         0x08    // to identify tL2C_LCB/BR
/* L2C_LPST_CB_CACHE_OK value (0x10) must be unused in BTM_LPST_CB_*) */
#define L2C_LPST_CB_CACHE_OK        0x10    // to inform SE the cache info is OK
#define L2C_LPST_CB_MAX_UNION       L2C_LPST_CB_CACHE_OK //use tL2C_LPST_CB union
#define L2C_LPST_CB_CACHE_INFO      0x20    // to inform partner there's cache info
#define L2C_LPST_CB_BR_INFO         0x40    // to inform partner there's a BR connection

extern void l2c_lpst_init (void);
extern UINT16 l2c_lpst_get_new_lcid(void);
extern void l2c_lpst_send_control_blocks (tL2C_CCB *p_ccb);
extern void l2c_lpst_clear_cb (void);
extern void l2c_lpst_set_connected_mask(UINT16 local_cid);
extern void l2c_lpst_set_cache_ok(void);
#endif
