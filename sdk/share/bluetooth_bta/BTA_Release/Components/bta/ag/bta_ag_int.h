...
typedef enum
{
    BTA_AG_SCO_MSBC_SETTINGS_T2 = 0, /* T2 preferred/default setting for mSBC */
    BTA_AG_SCO_MSBC_SETTINGS_T1      /* T1 safe setting for mSBC */
...
typedef struct
{
    ...
    tBTA_AG_SCO_MSBC_SETTINGS codec_msbc_settings; /* eSCO mSBC setting, T1 or T2 */
...
#if (BTA_HFP_VERSION >= HFP_VERSION_1_7 && BTA_HFP_HF_IND_SUPPORTED == TRUE)
    tBTA_AG_HF_IND      peer_hf_ind[BTA_AG_MAX_NUM_PEER_HF_IND];    /* Peer supported HF indicators */
    tBTA_AG_HF_IND      local_hf_ind[BTA_AG_MAX_NUM_LOCAL_HF_IND];  /* Local supported HF indicators */
#endif
} tBTA_AG_SCB;
...
#if BTA_AG_TEST == TRUE
    tBTM_ENH_ESCO_PARAMS    test_params;        /* ESCO parameters */
    BOOLEAN                 test_param_updated; /* params were updated to non-default */
#endif
...
extern void bta_ag_api_error(UINT16 event);
...
extern void bta_ag_audio_open_fail(tBTA_AG_SCB *p_scb, tBTA_AG_DATA *p_data);
extern void bta_ag_audio_close_fail(tBTA_AG_SCB *p_scb, tBTA_AG_DATA *p_data);
#if BTA_AG_TEST == TRUE
extern void bta_ag_set_esco_param(BOOLEAN set_reset, tBTM_ENH_ESCO_PARAMS *param);
#endif
