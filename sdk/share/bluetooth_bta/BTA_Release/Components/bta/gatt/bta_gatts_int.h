...
enum
{
    ...
    BTA_GATTS_API_ADD_SERVICE_BLOB_EVT
};
...
/* internal strucutre for GATTS register API  */
typedef struct
{
    BT_HDR                  hdr;
    tBT_UUID                app_uuid;
...
typedef struct
{
    BT_HDR                  hdr;
    tBTA_GATTS_IF           server_if;
    tGATT_DB_Service        service;
} tBTA_GATTS_API_ADD_SERVICE_BLOB;
...
typedef union
{
    ...
    tBTA_GATTS_API_ADD_SERVICE_BLOB api_add_service_blob;
} tBTA_GATTS_DATA;
...
extern void bta_gatts_add_service_blob(tBTA_GATTS_CB *p_cb, tBTA_GATTS_DATA * p_msg);
...
extern void bta_gatts_sys_disable(void);
