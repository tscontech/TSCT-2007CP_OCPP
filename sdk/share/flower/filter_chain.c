...
static void ite_filter_link(IteFilter *Outft, int outPin, IteFilter *Inft, int inPin, IteFcConf *fc_config)
{
    ...
    // Create Queue
    tmpQ_handler = ite_queue_new(fc_config->QSize, sizeof(IteQueueblk));
...
void ite_filterChain_stop(IteFChain *helper)
{
    ...
    while(currPtr != NULL) {
        ...
        if(currFilter->run == true) {
            ...
            // NOTICE: post semaphore, avoid thread blocking.
            if(fc_Conf->Semaphore) {
                if(currFilter->input[0].semHandle.__sem_lock) {
                    tmpSem = currFilter->input[0].semHandle;
                    sem_getvalue(&tmpSem, &sem_count);
                    if(sem_count <= 0) {
                        sem_post(&tmpSem);
                    }
                    DEBUG_PRINT("[%s] Filter(%d) has input[0].sem(%d)\n", __FUNCTION__, currFilter->filterDes.id, sem_count);
                }

                if(currFilter->input[1].semHandle.__sem_lock) {
                    tmpSem = currFilter->input[1].semHandle;
                    sem_getvalue(&tmpSem, &sem_count);
                    if(sem_count <= 0) {
                        sem_post(&tmpSem);
                    }
                    DEBUG_PRINT("[%s] Filter(%d) has input[1].sem(%d)\n", __FUNCTION__, currFilter->filterDes.id, sem_count);
                }
            }

            // thread join
            ret = pthread_join(currFilter->tID, NULL);
            DEBUG_PRINT("[%s]Filter(%d) stop therad(ret=%d)\n", __FUNCTION__, currFilter->filterDes.id, ret);            
        }
...
{
    IteFChain *currPtr = NULL, *tmpPtr = NULL, *headPtr = helper;
    IteFilter *tmpFilter = NULL;
    ...
    ite_flower_delete(headPtr);

    // find the next, because the head is not malloc
    ...
    memset(helper, 0, sizeof(IteFChain));
}
...
{
    IteFChain *currPtr = helper;
    IteFilter *preFilter = NULL, *tmpFilter = NULL;
    IteFcConf *fc_Conf = NULL;
    bool out_found = false;

    if(helper == NULL || inf == NULL || outf == NULL) {
    ...
    if(helper->filter == NULL) {
        printf("[%s] ERROR: filter chain must have one existed filter\n", __FUNCTION__);
    ...
    while(currPtr) {
        tmpFilter = currPtr->filter;
        if(tmpFilter->filterDes.id == outf->filterDes.id) {
            preFilter = tmpFilter;
            out_found = true;
            break;
        }
    ...
    if(out_found == false) {
        printf("[%s] ERROR: no such output filter\n", __FUNCTION__);

        return;
    }

    // reset currPtr to fc header
    currPtr = helper;
    ...
    // malloc fliter chain
    currPtr->nextPtr = ite_filterChain_add(inf);

    // find helper in Flower List
    fc_Conf = ite_flower_findFChainConfig(helper);

    // link filters
    ite_filter_link(preFilter, preFoutPin, inf, inputPin, fc_Conf);

    // inf filter want to run thread right now.
    if(bee) {
        IteFilter *runFilter = inf;
        DEBUG_PRINT("[%s]Filter(%d) link and run.\n", __FUNCTION__, runFilter->filterDes.id);
    
        //run filter thread
        if(runFilter->run == false) {
            runFilter->run = true;
            
            if(pthread_create(&runFilter->tID, NULL, runFilter->filterDes.process, (void *)runFilter) != 0) {
...
void ite_filterChain_A_unlink_B(bool bee, IteFChain *helper, IteFilter *outf, int preFoutPin, IteFilter *inf, int inputPin)
{
    ...
    IteFilter *currFilter = NULL, *preFilter = NULL, *tmpFilter = NULL, *inFilter = NULL;
    ...
    while(currFcPtr) {
        tmpFilter = currFcPtr->filter;
        if(outf->filterDes.id == tmpFilter->filterDes.id) {
            // find the output filter
            preFilter = tmpFilter;
            out_found = true;
        }
        ...
        if(nextFcPtr) {
            ...
            if(currFilter->filterDes.id == inf->filterDes.id) {
                inFilter = currFilter;
                in_found = true;
            }
        }
    ...
    if(in_found && out_found) {
        ...
        // unlink filters
        ite_filter_unlink(preFilter, preFoutPin, inFilter, inputPin, fc_Conf);

        // inf filter want to unlink and stop thread right now
        if(bee) {
            IteFilter *stopFilter = inf;
            sem_t tmpSem;
            int sem_count = 0, ret = 0;

            if(stopFilter->run == true) {
                // Set thread to stop
                stopFilter->run = false;
            
                // NOTICE: post semaphore, avoid thread blocking.
                if(fc_Conf->Semaphore) {
                    if(stopFilter->input[0].semHandle.__sem_lock) {
                        tmpSem = stopFilter->input[0].semHandle;
                        sem_getvalue(&tmpSem, &sem_count);
                        if(sem_count <= 0) {
                            sem_post(&tmpSem);
                        }
                        DEBUG_PRINT("[%s] Filter(%d) has input[0].sem(%d)\n", __FUNCTION__, stopFilter->filterDes.id, sem_count);
                    }

                    if(stopFilter->input[1].semHandle.__sem_lock) {
                        tmpSem = stopFilter->input[1].semHandle;
                        sem_getvalue(&tmpSem, &sem_count);
                        if(sem_count <= 0) {
                            sem_post(&tmpSem);
                        }
                ...
                // stop fitler thread
                ret = pthread_join(stopFilter->tID, NULL);
                DEBUG_PRINT("[%s]Filter(%d) stop therad(ret=%d)\n", __FUNCTION__, stopFilter->filterDes.id, ret);
            }
        }
    }
    else {
