...
static int get_local_ip_for_with_connect(int type, const char *dest, char *result){
    ...
    if (!(bool)ioctl(ITP_DEVICE_ETHERNET, ITP_IOCTL_IS_CONNECTED, NULL) || \
            !ioctl(ITP_DEVICE_ETHERNET, ITP_IOCTL_IS_AVAIL, NULL))
    {
        return -1;
    }
    ...
    for(p = res;p != NULL; p = p->ai_next) {
		if (p->ai_family == AF_INET) { // IPv4
          ...
          if (strncmp("eth0", info.name, IFNAMSIZ) == 0)
          {
              ipv4->sin_addr.s_addr = info.address;
          }
		}
	}
#endif
...
#ifdef CFG_NET_LWIP_2
    if (lwip_bind(sock, res->ai_addr, res->ai_addrlen) == -1){
        printf("bind error\n");
    }
#endif
	...
	if (p_addr->sa_family==AF_INET){
        ...
        strncpy(result, inet_ntoa(p_sin->sin_addr), LINPHONE_IPADDR_SIZE);
    }
