        ...
        if (driveStatus->disk == disk && driveStatus->avail)
        {
            if (j++ == partition)
            {
                if (format && par.partition_size > 0)
                {
                    ...
                    ret = ioctl(UG_DEVICE, ITP_IOCTL_FORMAT_PARTITION, (void*) ((disk << 8) | i));
#else
...
int ugUpgradeDoPartition(UGDevice* device)
{
    ...
    {
        ret = ioctl(UG_DEVICE, ITP_IOCTL_GET_PARTITION, &par);
        if (ret)
        {
    ...
    ret = ioctl(UG_DEVICE, ITP_IOCTL_MOUNT, (void*)par.disk);
    if (ret)
    {
...
repartition:
    ...
    ret = ioctl(UG_DEVICE, ITP_IOCTL_UNMOUNT, (void*)par.disk);
    if (ret)
    {
    ...
    ret = ioctl(UG_DEVICE, ITP_IOCTL_CREATE_PARTITION, &par);
    if (ret)
    {
...
#if defined(CFG_FS_LFS)
    for (i = 0; i < (int)device->partition_count; i++) {
        LOG_INFO "Format partition %d @ %s disk(s)...\n", i, device_table[device->device_type] LOG_END
        ret = ioctl(UG_DEVICE, ITP_IOCTL_FORMAT_PARTITION, (void*) ((par.disk << 8) | i));
        if (ret)
        {
            LOG_INFO "Format partition %d @ %s disk(s) fail: 0x%X\n", i, device_table[device->device_type], ret LOG_END
            goto end;
        }
    }
    ...
    LOG_INFO "Mount %s disk(s)...\n", device_table[device->device_type] LOG_END
    ret = ioctl(UG_DEVICE, ITP_IOCTL_FORCE_MOUNT, (void*)par.disk);
    if (ret)
    {
        LOG_ERR "Mount %s disk(s) fail: 0x%X\n", device_table[device->device_type], ret LOG_END
        goto end;
