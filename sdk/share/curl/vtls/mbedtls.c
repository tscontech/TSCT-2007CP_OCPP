...
static int bio_cf_read(void *bio, unsigned char *buf, size_t blen)
{
  ...
  printf("[bio_cf_read] Call this Function\r\n\r\n");
...
static CURLcode mbedtls_version_from_curl(int *mbedver, long version)
{
  ...
  printf("CURLE_SSL_CONNECT_ERROR from mbedtls_version_from_curl\r\n");
...
mbed_connect_step1(struct Curl_cfilter *cf, struct Curl_easy *data)
{
  ...
  if(ca_info_blob && verifypeer) {
    ...
    if(ret<0) {
      ...
      failf(data, "Error importing ca cert blob - mbedTLS: (-0x%04X) %s",
            -ret, errorbuf);
      printf("Error importing ca cert blob - mbedTLS:\r\n");
  ...
  if(ssl_cafile && verifypeer) {
    ...
    if(ret<0) {
      ...
      printf("Error reading ca cert file %s - mbedTLS: (-0x%04X) %s\r\n", ssl_cafile, -ret, errorbuf);
  ...
  if(ssl_capath) {
    ...
    if(ret<0) {
      ...
      printf("Error reading ca cert path %s - mbedTLS: (-0x%04X) %s\r\n", ssl_capath, -ret, errorbuf);
  ...
  if(ssl_cert) {
    ...
    printf("reading client cert file: %s\r\n", ssl_cert);
  ...
  if(ssl_config->key || ssl_config->key_blob) {
    if(ssl_config->key) {
      ...
      if(ret) {
        ...
        failf(data, "Error reading private key %s - mbedTLS: (-0x%04X) %s",
              ssl_config->key, -ret, errorbuf);
        printf("Error reading private key2 %s / errorbuf %s / ret : %d\r\n", ssl_config->key, errorbuf, ret);
        return CURLE_SSL_CERTPROBLEM;
      }
#else
      failf(data, "mbedtls: functions that use the filesystem not built in");
  ...
  if(ret) {
    ...
    printf("CURLE_SSL_CONNECT_ERROR from mbedTLS: ssl_config failed\r\n");
    return CURLE_SSL_CONNECT_ERROR;
  }
  ...
  if(mbedtls_ssl_setup(&backend->ssl, &backend->config)) {
    ...
    printf("CURLE_SSL_CONNECT_ERROR from mbedTLS: ssl_init failed\r\n");
    return CURLE_SSL_CONNECT_ERROR;
  }
  ...
  default:
    ...
    printf("CURLE_SSL_CONNECT_ERROR from Unrecognized parameter passed via CURLOPT_SSLVERSION\r\n");
    return CURLE_SSL_CONNECT_ERROR;
  }
  ...
  // printf("[mbed_connect_step1] mbedtls_ssl_set_bio()");
  ...
  if(ssl_config->primary.sessionid) {
    ...
    if(!Curl_ssl_getsessionid(cf, data, &old_session, NULL)) {
      ...
      if(ret) {
        ...
        printf("CURLE_SSL_CONNECT_ERROR from mbedtls_ssl_set_session returned -0x%x\r\n", -ret);
        return CURLE_SSL_CONNECT_ERROR;
      }
  ...
  }
  {
    ...
    if(!snihost || mbedtls_ssl_set_hostname(&backend->ssl, snihost)) {
      ...
      printf("CURLE_SSL_CONNECT_ERROR from Failed to set SNI\r\n");
      return CURLE_SSL_CONNECT_ERROR;
    }
  }
  ...
  if(cf->conn->bits.tls_enable_alpn) {
    ...
    *p = NULL;
    ...
    if(mbedtls_ssl_conf_alpn_protocols(&backend->config,
      ...
      printf("CURLE_SSL_CONNECT_ERROR from Failed setting ALPN protocols\r\n");
      return CURLE_SSL_CONNECT_ERROR;
    }
...
mbed_connect_step2(struct Curl_cfilter *cf, struct Curl_easy *data)
{
  ...
  if(ret == MBEDTLS_ERR_SSL_WANT_READ) {
    ...
    printf("CURLE_SSL_CONNECT_ERROR from ssl_handshake returned - mbedTLS: (-0x%04X) %s", -ret, errorbuf);
    return CURLE_SSL_CONNECT_ERROR;
  }
...
mbed_connect_step3(struct Curl_cfilter *cf, struct Curl_easy *data)
{
  ...
  if(ssl_config->primary.sessionid) {
    ...
    if(ret) {
      ...
      printf("CURLE_SSL_CONNECT_ERROR from mbedtls_ssl_get_session returned -0x%x\r\n", -ret);
      return CURLE_SSL_CONNECT_ERROR;
    }
    ...
    if(retcode) {
      ...
      printf("CURLE_SSL_CONNECT_ERROR from failed to store ssl session\r\n");
      return retcode;
    }
  }
