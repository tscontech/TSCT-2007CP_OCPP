...
extern uint32_t custom_timeout_u32t;
...
void curl_resp_timeout_set(uint32_t tv)
{
  custom_timeout_u32t = tv;
}
                                  ...
                                  struct curl_ws_frame **metap)
{
  ...
  *metap = NULL;
  ...
  while(!done) {
    ...
    // After Receive MSG, Excute Bellow Code .
...
{
  struct websocket *wsp = &data->req.p.http->ws;
  printf("[Curl_ws_done] Distroy Websocket\r\n");
  DEBUGASSERT(wsp);
