	...
	while(real_nr++<nr) 
	{
	    ...
	    switch(res)
	    {
	        ...
	        default:    //error, send touch-up
	        	...
	        	*/
	        	if(gpRaInfo->tpStatus == TOUCH_NO_CONTACT)	gpRaInfo->tpCurrINT = 0;
...
static void _raUpdateLastXY(struct ts_sample *smp)
{
	...
	if(smp!=NULL)
	{
		...
		if( smpChgFlag )
		{
			...
			memcpy((void*)gp_Sample ,(void*)smp, sizeof(struct ts_sample) * cFnr);
			lastFnr = (int)smp->finger;
			SendQueCnt++;
			//printf("inQ1:%d (%x,%x,%x)\n",SendQueCnt, gp_Sample->pressure, gp_Sample->x, gp_Sample->y);
