 ...
 ******************************************************************************/
int _readPointBuffer_tmp(int fd, unsigned char *Pbuf)
{
	int 			i2cret;
	
	i2cret = _readChipReg(fd, 0x00, Pbuf, 2);
	if(i2cret<0)
	{
	...
	{
		printf("[TOUCH ERROR].Read Point buffer fail(1)\n");
		return -1;
	}
	
	usleep(500);
	i2cret = _readChipReg(fd, 0x40, &Pbuf[2], 2);
	if(i2cret<0)
	{
		printf("[TOUCH ERROR].Read Point buffer fail\n");
		return -1;
...
int touch_getDummy(void){
	int i2cret;
	unsigned char buf[14]={0};
	i2cret = _readPointBuffer_tmp(g_tchDevFd,buf);
	if(i2cret >= 0)
		printf("[touch_getDummy] %d%d\r\n",buf[0]<<8+buf[1],buf[2]<<8+buf[3]);
	else
		printf("[touch_getDummy] Get error\r\n");
