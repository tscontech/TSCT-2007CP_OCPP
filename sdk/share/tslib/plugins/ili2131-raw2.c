...
/****************************************************************************
 * MACRO define of ILI2131  setting form kenny
 ****************************************************************************/
...
static void _tpDoPowerOnSeq_vendor(void)
{
	...
	//1.set "Reset pin" as output-high for 1ms	
	...
	usleep(1*1000); //High hold 1ms

    //2.set "Reset pin" output low for 10 ms
	ithGpioClear(gTpSpec.tpResetPin);
	usleep(10*1000); //low hold 10ms

    //3.set "INT pin" output HIGH for 300 ms
   	ithGpioSet(gTpSpec.tpResetPin);    	
...
static int _tpDoInitProgram_vendor(void)
{
    //TODO: To program touch IC for initial at first.(like IT7260)
    return 0;
}
...
{	
	...
	//only valid when buf[0] = 0x48(report ID)
	if(buf[0] != 0x48)   return (-1);
	...
	{
		for(i=0; i<gTpSpec.tpSampleNum; i++)	if(buf[(i * 5) + 1] & 0x40)	numOfTchPnt++;
	}
	...
	{		
		...
		{
			if(buf[(i * 5) + 1] & 0x40)	smp->pressure = 1;
			else	smp->pressure = 0;
				
			smp->id = (unsigned int)(buf[(i * 5) + 1] & 0xF)	; //touch id
			smp->finger = numOfTchPnt;
			smp->x = (short)(((unsigned int)(buf[i*5 + 3]&0x3F)<<8) + buf[i*5 + 2]);
			smp->y = (short)(((unsigned int)(buf[i*5 + 5]&0x3F)<<8) + buf[i*5 + 4]);
		...
		//exchange sample(only for ILI2131)
 ...
 */ 
static int ili2131_read(struct tslib_module_info *inf, struct ts_sample *samp, int nr)
{
...
static const struct tslib_ops ili2131_ops =
{
	ili2131_read,
};

TSAPI struct tslib_module_info *ili2131_mod_init(struct tsdev *dev, const char *params)
{
	...
	TSLIB_MODULE_INIT(ili2131_mod_init);
#endif
