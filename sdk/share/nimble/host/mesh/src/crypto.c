			       ...
			       u8_t *out_msg, size_t mic_size)
{
	...
	sys_endian_put_be16(0x0000, pmsg + 14);
	...
	memcpy(pmsg + 1, nonce, 13);
	sys_endian_put_be16(msg_len, pmsg + 14);
	...
	if (aad_len) {
		sys_endian_put_be16(aad_len, pmsg);
	...
	for (j = 0; j < blk_cnt; j++) {
		if (j + 1 == blk_cnt) {
			...
			pmsg[0] = 0x01;
			memcpy(pmsg + 1, nonce, 13);
			sys_endian_put_be16(j + 1, pmsg + 14);
			       ...
			       u32_t iv_index)
{
	...
	/* IV Index */
	sys_endian_put_be32(iv_index, &nonce[9]);
}
			  ...
			  const u8_t privacy_key[16])
{
	...
	sys_endian_put_be32(iv_index, &priv_rand[5]);
			     ...
			     u32_t iv_index)
{
	...
	sys_endian_put_be32((seq_num | ((u32_t)aszmic << 31)), &nonce[1]);

	sys_endian_put_be16(src, &nonce[5]);
	sys_endian_put_be16(dst, &nonce[7]);
...
int bt_mesh_virtual_addr(const u8_t virtual_label[16], u16_t *addr)
{
	...
	*addr = (sys_endian_get_be16(&tmp[14]) & 0x3fff) | 0x8000;
			...
			u8_t auth[8])
{
	...
	sys_endian_put_be32(iv_index, &msg[9]);
