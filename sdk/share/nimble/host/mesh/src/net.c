...
#define SRC(pdu)           (sys_endian_get_be16(&(pdu)[5]))
#define DST(pdu)           (sys_endian_get_be16(&(pdu)[7]))
...
static bool check_dup(struct os_mbuf *data)
{
	...
	val = sys_endian_get_be32(tail - 4) ^ sys_endian_get_be32(tail - 8);
...
int friend_cred_set(struct friend_cred *cred, u8_t idx, const u8_t net_key[16])
{
	...
	p[0] = 0x01;
	sys_endian_put_be16(lpn_addr, p + 1);
	sys_endian_put_be16(frnd_addr, p + 3);
	sys_endian_put_be16(cred->lpn_counter, p + 5);
	sys_endian_put_be16(cred->frnd_counter, p + 7);
