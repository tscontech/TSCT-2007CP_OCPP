...
static void prov_capabilities(const u8_t *data)
{
	...
	algorithms = sys_endian_get_be16(&data[1]);
	...
	output_action = sys_endian_get_be16(&data[6]);
	BT_DBG("Output OOB Action: 0x%04x", output_action);

	BT_DBG("Input OOB Size:    %u", data[8]);

	input_action = sys_endian_get_be16(&data[9]);
...
static int prov_auth(u8_t method, u8_t action, u8_t size)
{
	...
	case AUTH_METHOD_OUTPUT:
		...
		if (output == BT_MESH_DISPLAY_STRING) {
			...
			sys_endian_put_be32(num, &link.auth[12]);
...
static void prov_data(const u8_t *data)
{
	...
	net_idx = sys_endian_get_be16(&pdu[16]);
	flags = pdu[18];
	iv_index = sys_endian_get_be32(&pdu[19]);
	addr = sys_endian_get_be16(&pdu[23]);
