...
ble_hs_test_util_hci_verify_tx_disconnect(uint16_t handle, uint8_t reason)
{
    ...
    TEST_ASSERT(endian_get_le16(param + 0) == handle);
...
ble_hs_test_util_hci_verify_tx_create_conn(const struct hci_create_conn *exp)
{
    ...
    TEST_ASSERT(endian_get_le16(param + 0) == exp->scan_itvl);
    TEST_ASSERT(endian_get_le16(param + 2) == exp->scan_window);
    ...
    TEST_ASSERT(endian_get_le16(param + 13) == exp->conn_itvl_min);
    TEST_ASSERT(endian_get_le16(param + 15) == exp->conn_itvl_max);
    TEST_ASSERT(endian_get_le16(param + 17) == exp->conn_latency);
    TEST_ASSERT(endian_get_le16(param + 19) == exp->supervision_timeout);
    TEST_ASSERT(endian_get_le16(param + 21) == exp->min_ce_len);
    TEST_ASSERT(endian_get_le16(param + 23) == exp->max_ce_len);
}
    ...
    struct ble_hs_test_util_hci_num_completed_pkts_entry *entries)
{
    ...
    for (i = 0; i < num_entries; i++) {
        endian_put_le16(buf + off, entries[i].handle_id);
        off += 2;
        endian_put_le16(buf + off, entries[i].num_pkts);
        off += 2;
    }
                                               ...
                                               uint8_t status, uint8_t reason)
{
    ...
    buf[2] = status;
    endian_put_le16(buf + 3, conn_handle);
    buf[5] = reason;
