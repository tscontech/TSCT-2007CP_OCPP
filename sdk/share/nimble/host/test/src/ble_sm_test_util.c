...
ble_sm_master_id_parse(void *payload, int len, struct ble_sm_master_id *cmd)
{
    ...
    cmd->ediv = endian_get_le16(u8ptr);
    cmd->rand_val = endian_get_le64(u8ptr + 2);
}
...
ble_sm_master_id_write(void *payload, int len, struct ble_sm_master_id *cmd)
{
    ...
    endian_put_le16(u8ptr + 1, cmd->ediv);
    endian_put_le64(u8ptr + 3, cmd->rand_val);
}
...
ble_sm_test_util_verify_tx_lt_key_req_reply(uint16_t conn_handle, uint8_t *stk)
{
    ...
    TEST_ASSERT(endian_get_le16(param + 0) == conn_handle);
                                              ...
                                              uint16_t conn_handle)
{
    ...
    endian_put_le16(params, conn_handle);
                                     ...
                                     uint8_t *ltk)
{
    ...
    TEST_ASSERT(endian_get_le16(param + 0) == conn_handle);
    TEST_ASSERT(endian_get_le64(param + 2) == random_number);
    TEST_ASSERT(endian_get_le16(param + 10) == ediv);
