    ...
    uint16_t conn_handle, struct ble_gatt_find_s_test_entry *entries)
{
    ...
    for (i = 0; entries[i].inc_handle != 0; i++) {
        ...
        endian_put_le16(buf + off, entries[i].inc_handle);
        off += 2;

        endian_put_le16(buf + off, entries[i].start_handle);
        off += 2;

        endian_put_le16(buf + off, entries[i].end_handle);
        off += 2;

        if (entries[i].uuid->type == BLE_UUID_TYPE_16) {
            endian_put_le16(buf + off, ble_uuid_u16(entries[i].uuid));
            off += 2;
        }
    }
                                              ...
                                              uint16_t end_handle)
{
    ...
    uuid16 = endian_get_le16(om->om_data + BLE_ATT_READ_TYPE_REQ_BASE_SZ);
