...
TEST_CASE_SELF(ble_att_clt_test_rx_find_info)
{
    ...
    endian_put_le16(buf + off, 2);
    off += 2;
    endian_put_le16(buf + off, 0x000f);
    off += 2;
    ...
    endian_put_le16(buf + off, 3);
    off += 2;
    endian_put_le16(buf + off, 0x0010);
    off += 2;

    endian_put_le16(buf + off, 4);
    off += 2;
    endian_put_le16(buf + off, 0x0011);
    off += 2;
...
TEST_CASE_SELF(ble_att_clt_test_tx_read_mult)
{
    ...
    TEST_ASSERT(endian_get_le16(om->om_data + BLE_ATT_READ_MULT_REQ_BASE_SZ) == 1);
    TEST_ASSERT(endian_get_le16(om->om_data + BLE_ATT_READ_MULT_REQ_BASE_SZ + 2) == 2);
...
TEST_CASE_SELF(ble_att_clt_test_rx_read_mult)
{
    ...
    endian_put_le16(buf + BLE_ATT_READ_MULT_RSP_BASE_SZ + 0, 12);
    ...
    /*** Larger response. */
    endian_put_le16(buf + BLE_ATT_READ_MULT_RSP_BASE_SZ + 0, 12);
    endian_put_le16(buf + BLE_ATT_READ_MULT_RSP_BASE_SZ + 2, 43);
    endian_put_le16(buf + BLE_ATT_READ_MULT_RSP_BASE_SZ + 4, 91);
    rc = ble_hs_test_util_l2cap_rx_payload_flat(
        conn_handle, BLE_L2CAP_CID_ATT, buf,
