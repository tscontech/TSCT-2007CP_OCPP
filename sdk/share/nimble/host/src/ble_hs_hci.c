                       ...
                       struct ble_hs_hci_ack *out_ack)
{
	...
	//printf("expect op: %d, received op: %d\n", expected_opcode, out_ack->bha_opcode);
    if (rc == 0) {
                  ...
                  void *rsp, uint8_t rsp_len)
{
    ...
    ble_hs_hci_lock();
#ifdef DBG
	printf("cmd tx!!!!! opcode: %x, cmd_len: %d\n", opcode, cmd_len);
#endif
                           ...
                           uint8_t pb_flag)
{
    ...
    endian_put_le16(&hci_hdr.hdh_handle_pb_bc,
             ble_hs_hci_util_handle_pb_bc_join(handle, pb_flag, 0));
    endian_put_le16(&hci_hdr.hdh_len, OS_MBUF_PKTHDR(om)->omp_len);
    ...
    BLE_HS_LOG(DEBUG, "host tx hci data; handle=%d length=%d\n", handle,
               endian_get_le16(&hci_hdr.hdh_len));
#endif
