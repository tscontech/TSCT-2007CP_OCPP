    ...
    *format = 0;
    ...
    STAILQ_FOREACH(ha, &ble_att_svr_list, ha_next) {
        ...
        if (ha->ha_handle_id >= start_handle) {
            ...
            endian_put_le16(buf + 0, ha->ha_handle_id);
                                ...
                                uint16_t *err_handle)
{
    ...
    while (OS_MBUF_PKTLEN(*rxom) >= 2 && OS_MBUF_PKTLEN(txom) < mtu) {
         ...
         * mbuf.
         */
        handle = endian_get_le16((*rxom)->om_data);
        os_mbuf_adj(*rxom, 2);
                                        ...
                                        const ble_uuid_t *service_uuid)
{
    ...
    endian_put_le16(buf + 0, start_group_handle);
    endian_put_le16(buf + 2, end_group_handle);
