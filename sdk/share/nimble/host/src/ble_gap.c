...
ble_gap_rx_rd_rem_sup_feat_complete(const struct ble_hci_ev_le_subev_rd_rem_used_feat *ev)
{
    ...
    if ((conn != NULL) && (ev->status == 0)) {
        conn->supported_feat = endian_get_le32(ev->features);
    }
...

{
    ...
    if (params->itvl_min == 0 && params->itvl_max == 0) {
        ...
        endian_put_le24(cmd.pri_itvl_min, BLE_GAP_ADV_FAST_INTERVAL1_MIN);
        endian_put_le24(cmd.pri_itvl_max, BLE_GAP_ADV_FAST_INTERVAL2_MAX);
    } else {
        endian_put_le24(cmd.pri_itvl_min, params->itvl_min);
        endian_put_le24(cmd.pri_itvl_max, params->itvl_max);
    }
