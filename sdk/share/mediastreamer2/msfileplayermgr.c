...
static mblk_t *audiomgr_get_data(AudiomgrData *d){
        ...
        if (AD_r <= AD_w)
        {
            ...
            {
                rm = allocb(bsize, 0);
                ithInvalidateDCacheRange(codec_buf + AD_r, bsize);
                memcpy(rm->b_wptr, codec_buf + AD_r, bsize);
                rm->b_wptr += bsize;
                AD_r += bsize;
            }
        }
        ...
        { // AD_r > AD_w
            ...
            if (bsize)
            {
                ...
                if (szsec0)
                {
                    ithInvalidateDCacheRange(codec_buf + AD_r, szsec0);
                    memcpy(rm->b_wptr, codec_buf + AD_r, szsec0);
                }
                ithInvalidateDCacheRange(codec_buf, szsec1);
                memcpy(rm->b_wptr + szsec0, codec_buf, szsec1);
                rm->b_wptr += bsize;
                AD_r = szsec1;
            }
        }
...
static void audiomgr_process(MSFilter *f){
    ...
    if(d->state==MSPlayerPlaying){
        ...
        if(d->RP!=d->WP){
            ...
            //printf("%d\n",ms_bufferizer_get_avail(&d->buff));
