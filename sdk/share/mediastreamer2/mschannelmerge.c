...
static void *channel_merge_thread(void *arg) {
    ...
    while(s->start){
        ...
        if(s->bypass){//RL channel pass through
            err1 = ms_bufferizer_read(s->LBuf,iL->b_wptr,nbytes*2);
            err2 = ms_bufferizer_read(s->RBuf,iR->b_wptr,nbytes*2);
        }else{//RL channel switch
            err1 = ms_bufferizer_read(s->LBuf,iR->b_wptr,nbytes*2);
            err2 = ms_bufferizer_read(s->RBuf,iL->b_wptr,nbytes*2);            
        }
...
*/
static int channel_merge_set_bypass(MSFilter *f, void *arg){
	MSstate *s=(MSstate*)f->data;
    ms_mutex_lock(&s->mutex);
	s->bypass= *(int*)arg;
    ms_mutex_unlock(&s->mutex);
    printf("%s %d s->bypass=%d\n",__FUNCTION__,__LINE__,s->bypass);
    return 0;
}
static MSFilterMethod channel_merge_methods[]={
    {MS_FILTER_SET_BYPASS_MODE       , channel_merge_set_bypass },
};
