...
bool ituAnimationUpdate(ITUWidget* widget, ITUEvent ev, int arg1, int arg2, int arg3)
{
    ...
    else if (ev == ITU_EVENT_LAYOUT)
    {
		...
		if (animation)
		{
			...
			for (i = 0; i < count; i++)
			{
				ITUWidget* widget = (ITUWidget*)itcTreeGetChildAt(animation, i);
				if (widget)
				{
					int exc = (int)ituWidgetGetCustomData(widget);
					if ((widget->type == ITU_TEXT) && (exc == 0))
					{
						ITUText* text = (ITUText*)widget;
						int value = (text->fontWidth << 16) | text->fontHeight;
						ituWidgetSetCustomData(widget, value);
						printf("[Animation]backup font size for %s\n", widget->name);
					}
				}
			}
		}
