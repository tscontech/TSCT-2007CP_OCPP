...
float ituScaleCoverflowScaleFactor(ITUWidget* widget, int targetW)
{
	if (widget)
	{
		float factor = ((float)targetW) / ((float)widget->org_rect.width);
		//if (factor < 0.0)
		//	printf("ituScaleCoverflowScaleFactor error!\n");
		//else
		//	printf("widget %s cal factor %.3f\n", widget->name, factor);
		return factor;
	}
}
...
void ituScaleCoverflowScaleWidgetChild(ITUWidget* widget, float factor)
{
	...
	for (node = node->child; node; node = node->sibling)
	{
		...
		ITUWidget* child = (ITUWidget*)(node->child);
		
		if (child)
		{
			if (obj->type != ITU_BUTTON)
				ituScaleCoverflowScaleWidgetChild(child, factor);
		}

		ituScaleCoverflowSetChildSize(obj, factor);
		//printf("widget %s scale factor -> %.3f\n", obj->name, factor);
	}
	return;
}
    ...
    else if (ev == ITU_EVENT_LAYOUT)
    {
        if (scalecoverflow->itemWidth > 0 || scalecoverflow->itemHeight > 0 || scalecoverflow->itemPos > 0)
        {
            ...
            for (i = 0; i < count; ++i)
            {
				...
				ituScaleCoverflowScaleWidgetChild(child, ituScaleCoverflowScaleFactor(child, scalecoverflow->itemWidth));
            }            
        }
    }
			...
			for (i = 0; i < count; ++i)
			{
				...
				if (coverflow->coverFlowFlags & ITU_COVERFLOW_VERTICAL)
				{
					if (i == coverflow->focusIndex)
						...
						ituWidgetSetX(child, scalecoverflow->itemPos + (child->org_rect.width - scalecoverflow->itemWidth) / 2);
				}
				else
				{
					if (i == coverflow->focusIndex)
						...
						ituWidgetSetY(child, scalecoverflow->itemPos + (child->org_rect.height - scalecoverflow->itemHeight) / 2);
				}
			...
			if (1)//(coverflow->coverFlowFlags & ITU_COVERFLOW_CYCLE)
			{
				...
				for (i = 0; i < count; i++)
				{
					...
					else
					{
						...
						ituScaleCoverflowScaleWidgetChild(child, ituScaleCoverflowScaleFactor(child, orgWidth * scalecoverflow->factor / 100));
					}
				}
			}
