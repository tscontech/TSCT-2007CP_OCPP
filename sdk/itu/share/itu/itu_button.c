...
void ituButtonDraw(ITUWidget* widget, ITUSurface* dest, int x, int y, uint8_t alpha)
{
	...
	if ((btn->pressed) || (widget->active))
	{
		if ((!btn->pressSurf) || (!btn->focusSurf))
		{
			ituWidgetUpdate(btn, ITU_EVENT_LOAD, 0, 0, 0);
			//printf("Button %s load surf before draw. %d\n", widget->name, __LINE__);
		}
	}
#endif
	...
	if ((btn->pressed && btn->pressSurf) || (widget->active && btn->focusSurf))
	{
		...
		if ((btn->pressSurf) || (btn->focusSurf))
		{
			ituWidgetUpdate(btn, ITU_EVENT_RELEASE, 0, 0, 0);
			//printf("Button %s release surf after draw. %d\n", widget->name, __LINE__);
		}
#endif
	}
    else
