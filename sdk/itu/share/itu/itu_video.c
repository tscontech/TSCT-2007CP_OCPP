        ...
        if(!isOtherVideoPlaying && !isVideoWidgetStartPlaying)
        {
            ...
            if (video->playing && video->filePath[0] != '\0')
            {
                ...
                if (video->repeat)
                {
                    ...
                    pthread_create(&tid, &attr, polling_video_widget_status, (void *)video);
                }
                else
                {
                	mtal_pb_play();
...
#if (CFG_CHIP_FAMILY == 970 || CFG_CHIP_FAMILY == 9860) && (CFG_VIDEO_ENABLE)
    ituDrawVideoSurface_ex(dest, destx, desty, rect->width, rect->height);
#else
