...
bool ituCoverFlowClone(ITUWidget* widget, ITUWidget** cloned)
{
	...
	if (*cloned == NULL)
	{
		ITUWidget* newWidget = malloc(sizeof(ITUCoverFlow));
		if (newWidget == NULL)
			return false;

		memcpy(newWidget, widget, sizeof(ITUCoverFlow));
								...
								if (((local_fy + fix) < child->rect.y) && (local_inc > 0))
								{
									...
									while ((local_fy + calfix) < child->rect.y)
									{
										...
										if ((coverFlow->frame + calframe) >= coverFlow->totalframe)
											break;
										calfix = (int)(local_inc * ((float)(coverFlow->frame + calframe) / (float)coverFlow->totalframe));
										printf("calframe to %d [locay_fy %d calfix %d child_y %d]\n", calframe, local_fy, calfix, child->rect.y);
									}
								...
								if (((local_fx + fix) < child->rect.x) && (local_inc > 0))
								{
									...
									while ((local_fx + calfix) < child->rect.x)
									{
										...
										if ((coverFlow->frame + calframe) >= coverFlow->totalframe)
											break;
										calfix = (int)(local_inc * ((float)(coverFlow->frame + calframe) / (float)coverFlow->totalframe));
										printf("calframe to %d [locay_fx %d calfix %d child_x %d]\n", calframe, local_fx, calfix, child->rect.x);
									}
									coverFlow->frame += calframe;
									fix = calfix;
								}
