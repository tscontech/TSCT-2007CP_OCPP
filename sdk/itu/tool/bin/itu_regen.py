...
if (os.path.exists(workPath)):
    os.chdir(workPath)
    if (os.path.exists("DrawrockerGUIDesigner.exe")):
        print("Find DrawrockerGUIDesigner")
    else:
        print("Not found DrawrockerGUIDesigner.")
        sys.exit()
else:
...
try:
    ...
    print("\n\nRun DrawrockerGUIDesigner to export itu.")
    c = str(len(ts))
    percent = 0
    for i in range(len(ts)):
        if os.path.exists(ts[i]):
            ...
            os.system("DrawrockerGUIDesigner.exe")
