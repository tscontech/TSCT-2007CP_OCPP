...
#define MAX_BUF1_LEN            (BUFF_STRUCT_LEN + H5_RESERVED1 + (4 + (4 + 1021) + 2))
#define MAX_BUF2_LEN            (BUFF_STRUCT_LEN + H5_RESERVED1 + (4 + (4 + 256) + 2))
...
bool hci_h5_unpack(uint8_t *p_data_buf, uint16_t count)
{
    ...
    while (count)
    {
        ...
        case H5_W4_HDR:
            ...
            {
                h5_item = h5buf_alloc(4 + h5_parse_remainder + 2, 0);
            }
...
bool hci_h5_pack(uint8_t *p_data_buf, uint16_t data_length)
{
	...
	//        head = p_data_buf + 8; // weird cause bug
	//        length  = data_length - 8; // weird cause bug
	//#ifdef  RTK_COEX
