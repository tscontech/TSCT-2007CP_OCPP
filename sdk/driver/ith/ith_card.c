...
#if defined(CFG_GPIO_SD0_WIFI_POWER_ENABLE)
void ithWIFICardPowerOn(void)
{
    if (CFG_GPIO_SD0_WIFI_POWER_PIN == -1)
        return;

    ithGpioEnable((unsigned int)CFG_GPIO_SD0_WIFI_POWER_PIN);
    ithGpioSetOut((unsigned int)CFG_GPIO_SD0_WIFI_POWER_PIN);
#ifdef CFG_SD0_WIFI_DETECT_ACTIVE_HIGH
    ithGpioSet((unsigned int)CFG_GPIO_SD0_WIFI_POWER_PIN);
#else
    ithGpioClear((unsigned int)CFG_GPIO_SD0_WIFI_POWER_PIN);
#endif
}
