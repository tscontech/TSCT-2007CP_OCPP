...
static bool IT6122_Sys_ini(void)
{
    ...
    if (u8RevID == 0xC0 || u8RevID == 0xC1){
        ...
        //(*)
        i2c_write_byte(MPRxDevAddr, 0xC6, 0x3F);
        i2c_write_byte(MPRxDevAddr, 0xC7, 0x3F);
        //(*)
    }
	...
	//(*)
	i2c_write_byte(MPRxDevAddr, 0x05, 0x14);
	i2c_write_byte(MPRxDevAddr, 0x05, 0x04);
	usleep(5000);
	//(*)
    i2c_write_byte(MPRxDevAddr, 0x05, 0x00);
...
void ithLcdEnable(void)
{
    ...
    else if (gPanelType == ITH_LCD_MIPI)
    {
        ...
        ithLcdEnableHwFlip();
        usleep(2000); //avoid MIPI sync error
...
void ithLcdIT6122_ShowMParam(void)
{
    ...
    i2c_read_byte(MPRxDevAddr, 0x52, &u8Temp);
    u16MHSW = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x53, &u8Temp);
    u16MHSW += (u8Temp & 0x3F) << 8;

    i2c_read_byte(MPRxDevAddr, 0x50, &u8Temp);
    u16MHFP = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x51, &u8Temp);
    u16MHFP += (u8Temp & 0x3F) << 8;


    i2c_read_byte(MPRxDevAddr, 0x54, &u8Temp);
    u16MHBP = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x55, &u8Temp);
    u16MHBP += (u8Temp & 0x3F) << 8;

    i2c_read_byte(MPRxDevAddr, 0x56, &u8Temp);
    u16MHDEW = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x57, &u8Temp);
    u16MHDEW += (u8Temp & 0x3F) << 8;


    i2c_read_byte(MPRxDevAddr, 0x58, &u8Temp);
    u16MHVR2nd = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x59, &u8Temp);
    u16MHVR2nd += (u8Temp & 0x3F) << 8;

    u16MHBlank = u16MHFP + u16MHSW + u16MHBP;

    i2c_read_byte(MPRxDevAddr, 0x5C, &u8Temp);
    u16MVSW = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x5D, &u8Temp);
    u16MVSW += (u8Temp & 0x3F) << 8;

    i2c_read_byte(MPRxDevAddr, 0x5A, &u8Temp);
    u16MVFP = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x5B, &u8Temp);
    u16MVFP += (u8Temp & 0x3F) << 8;

    i2c_read_byte(MPRxDevAddr, 0x5E, &u8Temp);
    u16MVBP = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x5F, &u8Temp);
    u16MVBP += (u8Temp & 0x3F) << 8;

    i2c_read_byte(MPRxDevAddr, 0x60, &u8Temp);
    u16MVDEW = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x61, &u8Temp);
    u16MVDEW += (u8Temp & 0x3F) << 8;


    i2c_read_byte(MPRxDevAddr, 0x62, &u8Temp);
    u16MVFP2nd = u8Temp;
    i2c_read_byte(MPRxDevAddr, 0x63, &u8Temp);
    u16MVFP2nd += (u8Temp & 0x3F) << 8;
    ...
    ithPrintf("u16MHSW    = %d\r\n ", u16MHSW);
    ithPrintf("u16MHBP    = %d\r\n ", u16MHBP);
    ithPrintf("u16MHDEW   = %d\r\n ", u16MHDEW);
    ithPrintf("u16MHVR2nd = %d\r\n ", u16MHVR2nd);
    ithPrintf("u16MHBlank  = %d\r\n ", u16MHBlank);

    ithPrintf("u16MVFP    = %d\r\n ", u16MVFP);
    ithPrintf("u16MVSW    = %d\r\n ", u16MVSW);
    ithPrintf("u16MVBP   = %d\r\n ", u16MVBP);
    ithPrintf("u16MVDEW   = %d\r\n ", u16MVDEW);
    ithPrintf("u16MVFP2nd   = %d\r\n ", u16MVFP2nd);
    ithPrintf("u16MVTotal = %d\r\n ", u16MVTotal);
