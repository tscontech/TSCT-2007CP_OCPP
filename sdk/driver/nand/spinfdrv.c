...
static SPINF_CFG g_SpiNfCfgArray[] = 
{
	...
	{ "MX35LF2GE4AD", 0xC2, 0x26, 0x00, 2, 2048, 64, 2048, 128 },
	...
	{ "XT26G02C", 0x0B, 0x12, 0x00, 4, 2048, 64, 2048, 128 },
	{ "XT26G02E", 0x2C, 0x24, 0x00, 6, 2048, 64, 2048, 128 },
...
*/
static uint8_t gEsmtSprMapTable[] = { 0x20,0x21,0x22,0x23,0x24,0x25,0x26,0x27,
...
static uint8_t _checkEccStatus_MXIC(uint32_t blk, uint32_t ppo, uint8_t s)
{
    ...
    {
    	case 0x3: 
            printf("NfRdMxicE3, but can correct, (%x, %x, %x), DID=  %02x\n", blk, ppo, s, gSpiNfInfo.DID0);
            break;
        case 0x2:
...
static uint8_t _checkEccStatus_PRG(uint32_t blk, uint32_t ppo, uint8_t s)
{
    ...
    {
        case 0x3:
            printf("NfRdPrgE3(%x,%x,%x)\n",blk,ppo,s);
            //means ECC error and corrected (8 bits error)
            break;   
        case 0x2:
            printf("NfRdPrgE2(%x,%x,%x)\n",blk,ppo,s);
            //means ECC error and NOT be corrected
            return (23);
        case 0x1:
            //printf("NfRdPrgE1(%x,%x,%x)\n",blk,ppo,s);
            //means ECC error and corrected (1~7 bits error)
            break;
        case 0x0:
            //printf("NfRdPrgE0(%x,%x,%x)\n",blk,ppo,s);
            //mean no ECC error
            break;          
        default:
            printf("incorrect status of feature register!!!reg[C0]=%x\n",s);
...
others = Reserved.
*/
static uint8_t _checkEccStatus_ESMT(uint32_t blk, uint32_t ppo, uint8_t s)
{
    switch(s>>4)
    {
        case 0x5:
	...
	{	
	    ...
	    switch(gSpiNfInfo.MID)
	    {
	    	case 0xC8:
	   		...
	   		case 0x2C:
	   			rst = _checkEccStatus_ESMT(b,p,status);
	   			break;
...
static void _reMapSpare(uint8_t *ptr1,uint8_t *ptr2, uint8_t mapType)
{
	...
	if(gSpiNfInfo.MID==0xC2)	reMap = (uint8_t*)gGdSprMapTable;
		
	if(gSpiNfInfo.MID==0x2C)	reMap = (uint8_t*)gEsmtSprMapTable;
...
uint8_t spiNf_setQuadIoOperation(unsigned char value)
{
    ...
    //because toshiba has NO Quad-IO register, so skip it derectly.
    if(gSpiNfInfo.MID==0x2C)    return 0;
...
uint8_t spiNf_Initial(SPI_NF_INFO *info)
{
	...
	if(gSpiNfInfo.MID==0xC2)
	{
	    ...
	    if(gSpiNfInfo.DID0 == 0x35)   g2PlaneNandShift = 5;
	...
	if(gSpiNfInfo.MID==0x2C)
	{
	    ...
	    if(gSpiNfInfo.DID0==0x24)	g2PlaneNandShift = 4;
...
uint8_t spiNf_ByteRead(uint32_t blk, uint32_t ppo, uint8_t *dBuf, uint32_t offset, uint32_t rLen)
{
	...
	if (g2PlaneNandShift && (blk & 0x1))
	{
		SpiCmd[1] |= (1 << g2PlaneNandShift);
	}
...
uint8_t spiNf_PageProgram(uint32_t blk, uint32_t ppo, uint8_t *dBuf)
{
    ...
    if(g2PlaneNandShift)
    {
    	if( (g2PlaneNandShift!=4) && (g2PlaneNandShift!=5))	printf("SPI NAND Error: incorrect g2PlaneNandShift value, %x\n", g2PlaneNandShift);
    	else	SpiCmd[2] = ((blk&0x01)<<g2PlaneNandShift);	//if plane mode (0x00 | (blk&0x01)<<g2PlaneNandShift); 
    }
    else    SpiCmd[2] = 0x00;
