...
static int GetPackageSize(char* ftpurl, char* ftpid)
{
    CURL* curl = NULL;
...
// #endif
    ...
    {
        printf("curl_easy_getinfo(CURLINFO_CONTENT_LENGTH_DOWNLOAD) fail: %d, filesize: %d bytes\n", res, filesize);
        filesize = 0;
        goto end;
    }
...
static ITCStream* DownloadPackage(char* ftpurl, char* ftpid, int filesize)
{
    CURL *curl;
    CURLcode res;
    ...
    chunk.memory = malloc(filesize+1);    /* will be grown as needed by the realloc above */ 
    chunk.size = 0;   
    ...
    printf("[DownloadPackage] FTPID = %s, FTPURL = %s", ftpid, ftpurl);
...
// #endif
    ...
    if (CURLE_OK != res)
    {
        printf("curl_easy_perform() in DownloadPackage fail: %d\n", res);
        goto error;
    }
    else
    {
        printf("%lu bytes retrieved\n", (long)chunk.size);
        // fseek(ftpfile.stream, 0, SEEK_END);
        // getFiltSize = ftell(ftpfile.stream);

        // num = fread( buffer, sizeof( char ), NUM_ALPHA, ftpfile.stream);
    ...
    // itcArrayStreamOpen(&arrayStream, NULL, NULL);
...
static ITCStream*   OpenFtpPackage(char* path, char* id)
{
    int filesize;
    ...
    printf("size ftp url: %s\n", path);

    // get file size first
    // while (retry-- >= 0)
    // {
    //     filesize = GetPackageSize(path, id);
    //     printf("\r\n\r\n[%d]file size check \r\n",retry);
    //     if (filesize > 0)
    //         break;
    // }
    ...
    printf("down ftp url: %s\n", path);
    // download firmware
    while (retry-- >= 0)
    {
        fwStream = DownloadPackage(path, id, filesize);
        printf("\r\n\r\n[%d]file download check \r\n",retry);
        if (fwStream)
            break;
    };
...
int UpgradePackage(void)
{
    ...
    printf("\r\n Start remote package download ssssucucucuucuscusuc\r\n");
    fwStream = OpenFtpPackage(upgradeUrl, upgradeID);
    if (!fwStream)
    {
        ret = -1;
    ...
    if (ret)
    {
        fwUpdateVals.updateStep = UPDATE_STEP_INSTALFAIL;
...
int UpdateImg(void)
{
    ...
    ftpfile = fopen(CFG_PUBLIC_DRIVE ":/media/qr-code-test1.png", "wb");
    ...
    {
        /* specify URL to get */
        curl_easy_setopt(curl, CURLOPT_URL, upgradeUrl);
        /* specify IDPW to get */ 
        curl_easy_setopt(curl, CURLOPT_USERPWD, upgradeID);
        /* send all data to this function  */
        ...
        if (curl){
            curl_easy_cleanup(curl);
            printf("curl_easy_cleanup Finish!!\r\n");
            sleep(5);
        }
...
*/    
    ...
    // download from ftp server
    printf("Start remote IMG Download\r\n");
    fwStream = OpenFtpPackage(upgradeUrl, upgradeID);
    if (!fwStream)
    {
        ret = -1;
    ...
    //QRImage_size = itcStreamRead(fwStream, &QRImage, sizeof(uint32_t));
...
void UpgradeSetUrl(char* url, char* id, char* pw)
{
    if (url)
    {
        ...
        printf("#################UpgradeSetUrl# %s\r\n", upgradeUrl );

        strncpy(upgradeID, id, strlen(id));
        upgradeID[strlen(id)] = ':';
        // printf("#################upgradeID# %s\r\n", upgradeID );

        strncpy(&upgradeID[strlen(id)+1], pw, strlen(pw));
        upgradeID[strlen(id)+strlen(pw)+1] = '\0';
        printf("#################upgradeID# %s\r\n", upgradeID );
    }
    else
        upgradeUrl[0] = '\0';
