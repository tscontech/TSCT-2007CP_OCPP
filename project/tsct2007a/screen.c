...
static pthread_t sScreenSaverTask;
...
static void* ScreenSaverTask(void *arg){
    ...
    while(theConfig.screensaver_time != 0){
        usleep(200*1000);
        if(ScreenSaverCheck() && screenOff == false && EmgControl == false)
            ScreenOff();  // OK
    }
    sScreenSaverTask = 0;
}
...
void ScreenInit(void)
{
    ...
    if (sScreenSaverTask == 0)
	{
        CtLogYellow("[COMMON] create ground fault monitoring thread..\n");
        pthread_create(&sScreenSaverTask, NULL, ScreenSaverTask, NULL);
        pthread_detach(sScreenSaverTask);
    }
}