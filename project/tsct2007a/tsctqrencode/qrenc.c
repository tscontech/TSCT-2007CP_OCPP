...
static unsigned char bg_color[4] = {227, 237, 247, 255};
...
QRcode *encode(const unsigned char *intext, int length)
{
	...
	if(micro) {
		if(eightbit) {
			...
			printf("\r\nQR Encode Test Point 111111111111111111111111\r\n");
		} else {
			code = QRcode_encodeStringMQR((char *)intext, version, level, hint, casesensitive);
			printf("\r\nQR Encode Test Point 222222222222222222222222\r\n");
		}
	} else if(eightbit) {
		code = QRcode_encodeData(length, intext, version, level);
		printf("\r\nQR Encode Test Point 33333333333333333333333\r\n");
	} else {
		code = QRcode_encodeString((char *)intext, version, level, hint, casesensitive);
		printf("\r\nQR Encode Test Point 4444444444444444444444\r\n");
	}
