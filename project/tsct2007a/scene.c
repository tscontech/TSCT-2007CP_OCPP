...
void SceneInit(void)
{
	...
	ituSetRotation(ITU_ROT_0);//just for screen image
    #endif
...
int SceneRun(void)
{
    ...
    printf("[ScenRun] Enter infinity Loop\n");
    ...
    for (;;)
    {
        ...
        {
            if(ScreenIsOff())
                bOffScreenTouch = true;
            else
                bOffScreenTouch = false;
            switch (ev.type)
            {
            case SDL_KEYDOWN:
                ScreenSaverRefresh();
                if(bOffScreenTouch) break;
