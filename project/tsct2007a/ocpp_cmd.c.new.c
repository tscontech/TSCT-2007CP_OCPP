...
void DataProcCmd_DataTransCp(void)
{
	for(int i=0; i<=Rx_Msg.Payload_len ; i++){
		if(!strcmp(Rx_Msg.Payload[i].property_name, "status")) {
			if(!strcmp(Rx_Msg.Payload[i].property_contants, "Accepted")) {

			}
			else if(!strcmp(Rx_Msg.Payload[i].property_contants, "Rejected")) {
...
/////// 2024 12 02
	...
	if(MeterValQ[0].Meterval_Flg == METER_VAL_CLOCK_TYPE)
		sprintf((Tx_Msg.Payload[2].sub_Payload)->property_contants, "{\"timestamp\":\"%s\",\"sampledValue\":[{\"value\":\"%lu\",\"measurand\":\"Energy.Active.Import.Register\",\"context\":\"Sample.Clock\"}]}", MeterValQ[0].Time_Stamp, (MeterValQ[0].Sampled_Val * 10));
	else if(MeterValQ[0].Meterval_Flg == METER_VAL_SAMP_TYPE)
		sprintf((Tx_Msg.Payload[2].sub_Payload)->property_contants, "{\"timestamp\":\"%s\",\"sampledValue\":[{\"value\":\"%lu\",\"measurand\":\"Energy.Active.Import.Register\",\"context\":\"Sample.Periodic\",\"unit\":\"Wh\"}]}", MeterValQ[0].Time_Stamp, (MeterValQ[0].Sampled_Val * 10));
	else if(MeterValQ[0].Meterval_Flg == METER_VAL_TRIG_TYPE)
		sprintf((Tx_Msg.Payload[2].sub_Payload)->property_contants, "{\"timestamp\":\"%s\",\"sampledValue\":[{\"value\":\"%lu\",\"measurand\":\"Energy.Active.Import.Register\",\"context\":\"Trigger\"}]}", MeterValQ[0].Time_Stamp, (MeterValQ[0].Sampled_Val * 10));
}
...
void MakeDataCmd_Stat(uint8_t cntrNo)
{
	char temp_buf[50];
	Tx_Msg.Msg_type = MSG_TYPE_CALL;
	//Tx_Msg.Payload[2].subPayload_len = 0;
...
void MakeDataCmd_StopTs(void)
{
	...
	"{\"timestamp\":\"%s\",\"sampledValue\":[{\"value\":\"%lu\",\"measurand\":\"Energy.Active.Import.Register\",\"context\":\"Sample.Periodic\",\"unit\":\"Wh\"}]}", 
	StopTsConfig.Time_Stamp, (MeterValQ[0].Sampled_Val * 10));
	Tx_Msg.Payload_len++;
