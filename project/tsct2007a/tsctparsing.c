...
static bool Parse_Payload(char* buf, MSG_STRUCT* msg, uint16_t* index, size_t rlen)
{
    PAYLOAD* ptmpPayload;
    // uint8_t parsing_step=0, bObjDepth=0, str_index=0;
    uint8_t parsing_step=0, bObjDepth=0;
    ...
    for((*index)++; ( ((*index) < rlen) && (bArrStartFlg || (buf[*index] != ']')) ); (*index)++)
    {
		...
		else if(parsing_step == 1)  //property_name
        {
            str_index = 0;
			if(buf[*index] == '}')
            {
        ...
        else if(parsing_step == 5)  //property_contants
        {
			if(buf[*index] == '\"')
            {
                parsing_step = 6;
            ...
            {
                if(buf[*index] == '{')  //DataProcCmd_DataTransCp
                {
                    printf("Parse_Payload : for : parsing_step : 5 \n");
                    parsing_step = 7;
                    bArrStartFlg = true;

                    Pars_type[0] = TYPE_CODE_OBJ_STR;

                    memset((ptmpPayload + (*tempIndx))->DATATRANS_data, 0x00, sizeof((ptmpPayload + (*tempIndx))->DATATRANS_data));
                }
                else
                {
                    (ptmpPayload + (*tempIndx))->property_contants[str_index] = buf[*index];
		...
		// 	}
		// }
        else if(parsing_step == 7)  //DataProcCmd_DataTransCp
        {
            (ptmpPayload+(*tempIndx))->DATATRANS_data[str_index] = buf[*index];
            str_index++;

            if((buf_temp[0] == '}') && (buf[*index] == '\"'))
            {
                (*tempIndx)++;
                memset((ptmpPayload + (*tempIndx))->DATATRANS_data, 0x00, sizeof((ptmpPayload + (*tempIndx))->DATATRANS_data));
                (ptmpPayload+(*tempIndx))->subPayload_len = 0;
                parsing_step = 1;
                // printf("Parse_Payload : for : END \n");
            }

            // printf("Parse_Payload : for : %c, %c \n", buf_temp[0], buf[*index]);
            buf_temp[0] = buf[*index];
        }
    }
...
uint8_t Parse_Data(char* curl_ws_recv_buf, size_t rlen)
{
	...
	for(int i=0; i<=Rx_Msg.Payload_len; i++)
    {
        ...
        //printf("<< DATATRANS_data : %s >>\n", Rx_Msg.Payload[i].DATATRANS_data);
	}
	printf("\r\n]\r\n");
