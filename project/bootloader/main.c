...
static void* UgLedTask(void* arg)
{
    int gpio_pin = 44;
    ithGpioSetOut(44);
    ithGpioSetMode(44,ITH_GPIO_MODE0);
    // ithGpioSetOut(44);
    // ithGpioSetMode(44,ITH_GPIO_MODE0);
    stop_led = false;
...
static void DoUpgrade(void)
{
    ...
    if (upgradeFile)
    {
        ...
        LOG_INFO "Eject a USB stick and Reboot !!!!!!\n" LOG_END

        // exit(ret);
        while (1) {
			static bool bSet = false;
			
			if(bSet)	ithGpioSet(44);
			else		ithGpioClear(44);
...
{
        ithGpioSetOut(38);
        ithGpioSetMode(38, ITH_GPIO_MODE0);  //LCD_PWR_EN
        ithGpioSet(38);
        printf("[backlightResetTask] Turn on\n");
    usleep(100000);
    ioctl(ITP_DEVICE_BACKLIGHT, ITP_IOCTL_RESET, NULL);
    return NULL;
}
